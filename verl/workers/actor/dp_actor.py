...
def _chunked_logsumexp(x, dim=-1, chunk_size=4096):
    """Memory-efficient logsumexp over the last dim, computed in chunks.

    Mathematically identical to ``torch.logsumexp(x, dim, keepdim=True)`` but
    avoids materializing full-width fp32 intermediates (``x - max``, ``exp``)
    that dominate peak memory for large vocabularies (e.g. 248k for
    Qwen3.5-0.8B). Chunks are upcast to fp32 to match torch's internal
    accumulation dtype. Only ``dim`` (the last dim) is supported.
    """
    if dim != -1 and dim != x.dim() - 1:
        raise ValueError(f"_chunked_logsumexp only supports the last dim, got {dim}")
    *leading, vocab = x.shape
    flat = x.reshape(-1, vocab)
    # pass 1: per-row max
    row_max = flat[:, :chunk_size].float().max(dim=1, keepdim=True).values
    for s in range(chunk_size, vocab, chunk_size):
        row_max = torch.maximum(row_max, flat[:, s : s + chunk_size].float().max(dim=1, keepdim=True).values)
    # pass 2: per-row sum(exp(x - max))
    row_sum = torch.zeros_like(row_max)
    for s in range(0, vocab, chunk_size):
        row_sum = row_sum + torch.exp(flat[:, s : s + chunk_size].float() - row_max).sum(dim=1, keepdim=True)
    return (row_max + torch.log(row_sum)).reshape(*leading, 1)


def _support_membership(
    topk_indices: torch.Tensor, union_indices: torch.Tensor, slot_valid: torch.Tensor
) -> torch.Tensor:
    """Mark which union slots belong to ``topk_indices``'s own top-k support."""
    sorted_support, _ = torch.sort(topk_indices, dim=-1)
    support_size = sorted_support.size(-1)
    positions = torch.searchsorted(sorted_support, union_indices)
    in_range = positions < support_size
    hits = torch.gather(sorted_support, dim=-1, index=positions.clamp(max=support_size - 1)) == union_indices
    return slot_valid & in_range & hits


def _build_union_topk_indices(
    topk_indices_a: torch.Tensor, topk_indices_b: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Deduplicated union of two ``(bs, response_len, k)`` top-k index tensors.

    Each world contributes its own teacher top-k support and every distribution of the
    dual-world loss is gathered on the union of the two. Sorting the merged supports
    ascending gives each deduplicated entry a stable slot, which is what lets the caller
    mark ``member_a`` / ``member_b`` for the per-world distillation terms.

    Returns:
        union: (bs, response_len, K) int64 ascending union, ``K = max union size`` so a batch
            whose supports overlap (or a single sample) carries no padding at all. Any slot
            still beyond a row's own union size repeats that row's first member, which keeps
            the gathers in range and the gathered log-probs real.
        member_a / member_b: (bs, response_len, K) bool, True where the slot belongs to that
            world's own top-k. Padding slots are False in both masks.
    """
    if topk_indices_a.size(-1) != topk_indices_b.size(-1):
        raise ValueError(
            f"union requires both supports to have the same k, got {topk_indices_a.size(-1)} "
            f"and {topk_indices_b.size(-1)}"
        )
    merged, _ = torch.sort(torch.cat([topk_indices_a, topk_indices_b], dim=-1), dim=-1)
    is_first_occurrence = torch.ones_like(merged, dtype=torch.bool)
    is_first_occurrence[..., 1:] = merged[..., 1:] != merged[..., :-1]
    # slot each entry lands in; duplicates share the rank of the last kept entry, which is
    # harmless because a forward fill gives them the value of that same kept entry.
    ranks = torch.cumsum(is_first_occurrence.to(torch.long), dim=-1) - 1
    kept_values = torch.cummax(torch.where(is_first_occurrence, merged, -1), dim=-1).values
    union = torch.zeros_like(merged)
    union.scatter_(-1, ranks, kept_values)

    keep = int(ranks[..., -1:].max().item()) + 1  # widest union of the batch: trims the rest
    union = union[..., :keep]
    slot_ids = torch.arange(keep, device=merged.device).view(*([1] * (merged.dim() - 1)), -1)
    slot_valid = slot_ids < (ranks[..., -1:] + 1)
    slot_valid = slot_valid.expand_as(union)
    union = torch.where(slot_valid, union, union[..., :1]).contiguous()
    return (
        union,
        _support_membership(topk_indices_a, union, slot_valid),
        _support_membership(topk_indices_b, union, slot_valid),
    )


def _log_probs_on_support(response_logits: torch.Tensor, support_indices: torch.Tensor) -> torch.Tensor:
    """Log-softmax values of ``response_logits`` gathered on ``support_indices``.

    Same normalization the single-world top-k path uses: ``logit - logsumexp(logits)``.
    """
    return torch.gather(response_logits, dim=-1, index=support_indices) - _chunked_logsumexp(
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_teacher(self) -> None:
        ...
        if not self_distillation_cfg or loss_mode != "cwad":
            return
    ...
    def _forward_micro_batch(
        ...
        Returns:
            dict[str, torch.Tensor]:
                ...
                if return_response_logits is True (and top-k is active):
                    response_logits: (bs, response_len, vocab_size), raw logits over the
                        response span, used by the dual-world loss to re-gather all four
                        distributions on the union of the two teachers' top-k supports.
        """
        ...
        if return_response_logits and use_topk and self.use_ulysses_sp:
            raise NotImplementedError(
                "return_response_logits (dual-world distillation) does not support ulysses sequence parallelism."
            )
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    if use_topk:
                        ...
                        logsumexp_rmpad = _chunked_logsumexp(logits_rmpad, dim=-1)
                        topk_logps_rmpad = topk_logits_rmpad - logsumexp_rmpad

                        if return_response_logits and is_mask_all_zero:
                            # fully masked batch: no rmpad row to recover, keep the layout only
                            response_logits = logits_rmpad.new_zeros(
                                (batch_size, response_length, logits_rmpad.size(-1))
                            )
                        elif return_response_logits:
                            # mirror full_topk_indices: recover the (bs, response_len) rows of the
                            # flattened (bs, seqlen) layout, then map them onto the rmpad rows.
                            if response_start_idx is None:
                                response_positions = (
                                    torch.arange(
                                ...
                                response_positions = self._build_response_positions(
                                    response_start_idx=response_start_idx.to(device=logits_rmpad.device),
                                    response_length=response_length,
                                    seqlen=seqlen,
                                )
                            flat_response_positions = (
                                torch.arange(batch_size, device=logits_rmpad.device).unsqueeze(1) * seqlen
                                + response_positions
                            ).reshape(-1)
                            # invert the unpad permutation so each response position yields its rmpad row
                            rmpad_rows = torch.zeros(
                                batch_size * seqlen, device=logits_rmpad.device, dtype=torch.long
                            )
                            rmpad_rows[indices.reshape(-1)] = torch.arange(
                                indices.numel(), device=logits_rmpad.device
                            )
                            # Under dynamic batching a response slot can sit past the end of a
                            # short (EOS-terminated) rollout, so it has no rmpad row. Such slots
                            # are already zeroed in response_mask and never reach the loss; zero
                            # the gathered logits too so the value is deterministic instead of
                            # silently borrowing rmpad row 0.
                            valid_response_slots = attention_mask.reshape(-1)[flat_response_positions].bool()
                            response_logits = logits_rmpad[rmpad_rows[flat_response_positions]].view(
                                batch_size, response_length, logits_rmpad.size(-1)
                            )
                            response_logits = response_logits * valid_response_slots.view(
                                batch_size, response_length, 1
                            ).to(response_logits.dtype)
                ...
                if self.use_fused_kernels:
                    ...
                    if return_response_logits:
                        # already (bs, response_len, vocab_size); log_probs does not modify it
                    ...
                    if use_topk:
                        ...
                        logsumexp = _chunked_logsumexp(logits, dim=-1)
            ...
            if return_response_logits and use_topk:
    ...
    def update_policy(self, data: DataProto):
        ...
        if self_distillation_enabled:
            ...
            # Privileged dual-world worlds carry their own prompt tensors. They are
            # optional -- absent on the plain two-image layout -- so select them only
            # when the trainer actually emitted them.
            for _prefix in ("teacher_a", "student_b"):
                for _suffix in ("input_ids", "attention_mask", "position_ids", "response_start_idx"):
                    _key = f"{_prefix}_{_suffix}"
                    if _key in data.batch.keys():
                        select_keys.append(_key)
        ...
        has_world_b_multi_modal_inputs = self._has_non_empty_multi_modal_inputs(
            data.non_tensor_batch.get("world_b_multi_modal_inputs")
        )
        # Privileged CWAD carries two extra image columns (the teacher's world-A
        # view and the student's world-B view); both must survive `select` or the
        # strict-privilege variant silently degrades to the unprivileged layout.
        has_teacher_a_multi_modal_inputs = self._has_non_empty_multi_modal_inputs(
            data.non_tensor_batch.get("teacher_a_multi_modal_inputs")
        )
        has_student_b_multi_modal_inputs = self._has_non_empty_multi_modal_inputs(
            data.non_tensor_batch.get("student_b_multi_modal_inputs")
        )
        ...
        if has_world_b_multi_modal_inputs:
            non_tensor_select_keys.append("world_b_multi_modal_inputs")
        if has_teacher_a_multi_modal_inputs:
            non_tensor_select_keys.append("teacher_a_multi_modal_inputs")
        if has_student_b_multi_modal_inputs:
            non_tensor_select_keys.append("student_b_multi_modal_inputs")
        ...
        if self_distillation_enabled:
            ...
            metrics["actor/cwad_loss"] = 0.0
            metrics["actor/cwad_loss_weighted"] = 0.0
        distill_dump_chunks = []
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    distillation_topk_source = self_distillation_cfg.get("distillation_topk_source", "student")
                    # Dual-world (CWAD): world A is the original image, seen at student resolution by
                    # the student (model_inputs) and at original resolution by the teacher
                    # (teacher_inputs), while world B is the counterfactual edit both sides see
                    # through world_b_inputs. The rollout only saw world A, so besides the usual
                    # student@A / teacher@B passes we also need student@B and teacher@A, all
                    # gathered on the union of the two teachers' top-k supports.
                    use_dual_world = (
                        self_distillation_enabled
                        and self_distillation_cfg.get("dual_world", False)
                        and bool(distill_topk)
                        and distillation_topk_source == "teacher"
                    )
                    teacher_a_topk_logps = None
                    teacher_b_topk_logps = None
                    member_a = None
                    member_b = None
                    ...
                    if self_distillation_enabled:
                        ...
                        if use_dual_world:
                            # Two-image layout: world A is the `images` column (which the trainer
                            # already turned into model_inputs) and world B is teacher_image_key
                            # (teacher_inputs). Both worlds share the teacher prompt/response
                            # tensors; only the image each side scores differs.
                            world_b_inputs = teacher_inputs
                            # Privileged variant (A6): world A and world B each carry a second
                            # image at the other resolution. Their prompt prefixes cannot be
                            # shared with the teacher's -- a different-resolution image expands
                            # to a different number of vision tokens, and the model requires the
                            # prompt's image tokens and the pixel inputs to agree -- so each
                            # privileged world brings its own tokenized prefix. A missing tensor
                            # falls back to the unprivileged behaviour.
                            def _privileged_inputs(prefix: str) -> dict:
                                """One privileged world: its own prompt prefix, shared response."""
                                world_inputs = {
                                    "responses": model_inputs["responses"],
                                    ...
                                    "response_start_idx": model_inputs[f"{prefix}_response_start_idx"],
                                }
                                multi_modal_key = f"{prefix}_multi_modal_inputs"
                                if multi_modal_key in model_inputs:
                                    world_inputs["multi_modal_inputs"] = model_inputs[multi_modal_key]
                                return world_inputs

                            teacher_a_inputs = (
                                _privileged_inputs("teacher_a")
                                if "teacher_a_input_ids" in model_inputs
                                else teacher_inputs
                            )
                            student_b_inputs = (
                                _privileged_inputs("student_b")
                                if "student_b_input_ids" in model_inputs
                                else teacher_inputs
                            )
                        ...
                        if use_dual_world:
                            # both teachers first: their supports only become known here, and the
                            # retained response logits let us re-gather them on the union below.
                            with torch.no_grad():
                                teacher_forward_start = time.perf_counter()
                                teacher_a_outputs = self._forward_micro_batch(
                                    teacher_a_inputs,  # world A (privileged: teacher resolution)
                                    temperature=temperature,
                                ...
                                teacher_b_outputs = self._forward_micro_batch(
                                    world_b_inputs,  # world B image
                                    temperature=temperature,
                                    calculate_entropy=False,
                                    return_all_logps=False,
                                    distill_topk=distill_topk,
                            ...
                            teacher_a_topk_indices = teacher_a_outputs.get("topk_indices")
                            teacher_b_topk_indices = teacher_b_outputs.get("topk_indices")
                            if teacher_a_topk_indices is None or teacher_b_topk_indices is None:
                                raise ValueError("dual-world distillation did not return topk_indices.")
                            # TEMP DEBUG: confirm the two worlds really are different --
                            # different prompt prefixes AND different teacher supports.
                            if not getattr(self, "_hint_probe_done", False):
                                self._hint_probe_done = True
                                _a = teacher_a_topk_indices.reshape(-1, teacher_a_topk_indices.size(-1))
                                _b = teacher_b_topk_indices.reshape(-1, teacher_b_topk_indices.size(-1))
                                _eq = (_a == _b).all(dim=-1).float().mean().item()
                                print(
                                    f"[DEBUG hint] has_a={'teacher_a_input_ids' in model_inputs} "
                                    f"has_b={'student_b_input_ids' in model_inputs} | "
                                    f"teacher_a_seqlen={model_inputs['teacher_a_input_ids'].shape[1] if 'teacher_a_input_ids' in model_inputs else -1} "
                                    f"teacher_b_seqlen={model_inputs['teacher_input_ids'].shape[1]} "
                                    f"student_a_seqlen={model_inputs['input_ids'].shape[1]} "
                                    f"student_b_seqlen={model_inputs['student_b_input_ids'].shape[1] if 'student_b_input_ids' in model_inputs else -1} | "
                                    f"topk A==B rowmatch={_eq:.4f}",
                                    flush=True,
                                )
                            teacher_topk_indices, member_a, member_b = _build_union_topk_indices(
                                teacher_a_topk_indices, teacher_b_topk_indices
                            )
                            union_sizes = (member_a | member_b).sum(-1).float()
                            micro_batch_metrics["self_distillation/cwad_union_size_mean"] = (
                                union_sizes.mean().detach().item()
                            )
                            micro_batch_metrics["self_distillation/cwad_union_size_max"] = (
                                union_sizes.max().detach().item()
                            )
                            with torch.no_grad():
                                teacher_a_topk_logps = _log_probs_on_support(
                                    teacher_a_outputs.pop("response_logits"), teacher_topk_indices
                                )
                                teacher_b_topk_logps = _log_probs_on_support(
                                    teacher_b_outputs.pop("response_logits"), teacher_topk_indices
                                )
                            del teacher_a_outputs, teacher_b_outputs
                        elif distill_topk and distillation_topk_source == "teacher":
                            with torch.no_grad():
                                teacher_forward_start = time.perf_counter()
                            ...
                            if teacher_topk_indices is None:
                                raise ValueError("teacher top-k distillation did not return topk_indices.")
                    ...
                    outputs = self._forward_micro_batch(
                        ...
                        distill_topk=distill_topk if distillation_topk_source == "student" else None,
                    ...
                    if self_distillation_enabled:
                        if use_dual_world:
                            # student @ world B on the same union support; gradients flow into the
                            # student, which is what makes the two worlds comparable.
                            student_b_forward_start = time.perf_counter()
                            student_b_outputs = self._forward_micro_batch(
                                student_b_inputs,
                                temperature=temperature,
                                calculate_entropy=False,
                                return_all_logps=False,
                            ...
                            student_b_forward_time = time.perf_counter() - student_b_forward_start
                            stage_wall_time_totals["timing_s/update_actor/student_forward"] += student_b_forward_time
                            student_b_topk_logps = student_b_outputs["topk_logps"]
                            del student_b_outputs
                        elif teacher_outputs is None:
                            with torch.no_grad():
                                teacher_forward_start = time.perf_counter()
                                teacher_outputs = self._forward_micro_batch(
                                    teacher_inputs,
                        ...
                        # the dump expects a single teacher support, so it is skipped in dual-world mode
                        dump_log_probs = self_distillation_cfg.get("log_prob_dump_dir", None) and not use_dual_world
                        if dump_log_probs:
                            if distill_topk:
                        ...
                        if use_dual_world:
                            # student_topk_logps is student@A on the union, i.e. the S_A of CWAD.
                            cwad_loss, cwad_metrics = compute_cwad_loss(
                                student_a_topk_log_probs=student_topk_logps,
                                student_b_topk_log_probs=student_b_topk_logps,
                                teacher_a_topk_log_probs=teacher_a_topk_logps,
                                teacher_b_topk_log_probs=teacher_b_topk_logps,
                                member_a=member_a,
                                member_b=member_b,
                                response_mask=response_mask,
                                cwad_lambda=self_distillation_cfg.cwad_lambda,
                                cwad_tau=self_distillation_cfg.cwad_tau,
                            ...
                            cwad_loss, cwad_metrics = compute_self_distillation_loss(
                                student_log_probs=log_prob,
                        ...
                        cwad_metrics["self_distillation/empty_target_batch"] = self_distillation_mask.sum().item() == 0
                        micro_batch_metrics.update(cwad_metrics)

                        if policy_fallback_mask is not None and policy_fallback_mask.any().item():
                            if advantages is None:
                                raise ValueError(
                                    "Mixed distillation/GRPO fallback requires advantages for samples without teacher images."
                                )
                    ...
                    if self_distillation_enabled:
                        metrics["actor/cwad_loss"] += cwad_loss.detach().item() * loss_scale_factor
                        metrics["actor/cwad_loss_weighted"] += cwad_loss.detach().item() * loss_scale_factor
