...
# Upper bound on an image's pixel area, applied before the processor sees it.
#
# The model's preprocessor_config.json allows longest_edge=16777216 (16.7 Mpx),
# so images are effectively never downscaled, and a vision token costs a fixed
# patch, so token count scales with pixel area. The dual-world dataset mixes two
# kinds of world-B image: counterfactual edits, which are always 1024x1024, and
# fallbacks that keep the original teacher resolution -- measured as high as
# 7.47 Mpx, i.e. ~7x the edits and roughly 7,000 vision tokens instead of 1,000.
# Those rows produce sequences several times longer than every other sample,
# which is what blew the actor's forward activation budget at step 454.
#
# Capping here keeps every world-B image comparable in cost to an edit without
# touching the dataset: RLHFDataset opens images at __getitem__ time, so this
# takes effect on the next run and the prefilter cache (which stores paths, not
# pixels) stays valid -- shrinking images can only make a prompt shorter, and
# the cache was built by filtering on the uncapped ones.
MAX_IMAGE_PIXELS = int(os.environ.get("CWAD_MAX_IMAGE_PIXELS", str(1024 * 1024)))


def _cap_pixels(image: Image.Image, max_pixels: int = MAX_IMAGE_PIXELS) -> Image.Image:
    if max_pixels <= 0:
        return image
...
def process_image(image: dict | Image.Image, image_patch_size: int = 14) -> Image.Image:
    if isinstance(image, Image.Image):
        return _cap_pixels(image.convert("RGB"))
    ...
    # fetch_image accepts a path, bytes or a PIL image; resolve to a PIL image
    # first so the cap can be applied whichever form the sample carried.
    if not isinstance(loaded, Image.Image):
        loaded = Image.open(loaded)

    image["image"] = _cap_pixels(loaded.convert("RGB"))
