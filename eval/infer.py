...
def acquire_slot(requests_per_minute):
    """Block until this request fits in the trailing 60s window (workers share one budget)."""
    if not requests_per_minute:
        return
    while True:
        with _rate_lock:
            now = time.time()
            while _rate_stamps and now - _rate_stamps[0] > 60.0:
                _rate_stamps.popleft()
            if len(_rate_stamps) < requests_per_minute:
                _rate_stamps.append(now)
                return
            wait = 60.0 - (now - _rate_stamps[0]) + 0.05
        time.sleep(wait)
...
def image_to_data_uri(path_str, benchmark, image_scale_divisor=1):
    if image_scale_divisor > 1 or benchmark in VSTAR_BENCHMARKS:
        img = Image.open(path_str).convert("RGB")
        if image_scale_divisor > 1:
            target_size = (
                max(1, img.size[0] // image_scale_divisor),
                max(1, img.size[1] // image_scale_divisor),
            )
        ...
        while (
            benchmark in VSTAR_BENCHMARKS
            and len(byte_data) > VSTAR_MAX_IMAGE_SIZE
            and img.size[0] > 100
            and img.size[1] > 100
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help="throttle all workers to this many API requests per minute (0 = unlimited)",
    )
    parser.add_argument(
        "--image_scale_divisor",
        default=1,
        type=int,
        help="Physically divide image width and height by this value with LANCZOS before inference.",
    )
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Comma-separated optional params to NOT send (e.g. top_p,seed,temperature) for APIs "
             "that reject them (kimi-k3 rejects top_p).",
    )
    args = parser.parse_args()
    if args.image_scale_divisor < 1:
        parser.error("--image_scale_divisor must be at least 1")
    omit_params = {x.strip() for x in args.omit_params.split(",") if x.strip()}
    if args.api_key in ("", "EMPTY") and os.environ.get("EVAL_API_KEY"):
        # /proc is not hidepid on this cluster: keep third-party keys out of argv
        args.api_key = os.environ["EVAL_API_KEY"]
    ...
    print(f"Image scale divisor: {args.image_scale_divisor}")
    if not todo_data:
    ...
    def run_one(item):
        ...
        data_uri = image_to_data_uri(img_path, benchmark, args.image_scale_divisor)
        messages = [
            {
                "role": "user",
                "content": [
        ...
        for attempt in range(1, args.max_retries + 1):
            try:
                acquire_slot(args.requests_per_minute)
                req = {"model": args.model_id, "messages": messages, "max_tokens": args.max_tokens}
                if "temperature" not in omit_params:
                    req["temperature"] = args.temperature
                if "top_p" not in omit_params:
                    req["top_p"] = args.top_p
                if "seed" not in omit_params:
                    req["seed"] = args.seed
                ...
                raw_model_answer = (choice.message.content or "").strip()
                ...
                if rc is None and getattr(choice.message, "model_extra", None):
                    rc = choice.message.model_extra.get("reasoning_content")
                if isinstance(rc, str):
                    thinking_chars = len(rc)
                if not raw_model_answer and attempt < args.max_retries:
                    # empty content (e.g. thinking model burned the whole budget) -> retry
                    time.sleep(2.0)
                    continue
                ...
                if attempt == args.max_retries:
                    ...
                    txt = str(e).lower()
                    limited = any(k in txt for k in ("429", "rate limit", "ratelimit", "too many", "overloaded"))
                    time.sleep(60.0 if limited else 1.0)
        record = dict(item)
        record["sample_uid"] = sample_uid
        record["model_answer"] = model_answer
        record["evaluation_image_scale_divisor"] = args.image_scale_divisor
        record["finish_reason"] = finish_reason
        record["usage"] = usage
        record["thinking_chars"] = thinking_chars
        return record
