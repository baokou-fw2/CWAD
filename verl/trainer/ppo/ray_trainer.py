...
from verl.utils.dataset.vision_utils import _cap_pixels as _cap_image_pixels
...
def should_reuse_rollout_log_probs_as_old_log_probs(config, batch: DataProto) -> bool:
    ...
    This preserves the current distillation loss mode while skipping the extra old_log_prob
...
class RayPPOTrainer:
    ...
    def _compute_or_extract_reward(
        ...
        if reward_fn is None and self._use_reward_free_teacher_cwad():
    ...
    def _normalize_teacher_image(image: Any) -> Image.Image:
        # Teacher and world-B images are opened here rather than through the
        # dataset's process_image, so the pixel cap has to be applied here too
        # or those views keep their full original resolution.
        if isinstance(image, Image.Image):
            return _cap_image_pixels(image.convert("RGB"))
        if isinstance(image, str):
            with Image.open(image) as pil_image:
                return _cap_image_pixels(pil_image.convert("RGB"))
        if isinstance(image, dict):
            if "image" in image:
                return RayPPOTrainer._normalize_teacher_image(image["image"])
            image_bytes = image.get("bytes", None)
            if image_bytes is not None:
                return _cap_image_pixels(Image.open(BytesIO(image_bytes)).convert("RGB"))
            if "path" in image:
    ...
    def _prepare_hint_teacher_messages(
        self,
    ...
    def _raise_if_response_contains_visual_special_tokens(
        ...
        message = (
            "Response contains vision special tokens before the teacher forward pass, which would corrupt "
    ...
    @staticmethod
    def _images_from_batch(batch, key: str, index: int) -> list:
        """Read one image column for one sample, normalised to a plain list."""
        images = batch.non_tensor_batch[key][index]
    ...
    def _pad_prompt_list(self, lists: list, batch_size: int, device) -> tuple:
        """Pad one world's per-sample prompt inputs into batch tensors.

        `lists` is [input_ids, attention_mask, position_ids, response_start_idx,
        multi_modal_inputs]. World-specific prompt prefixes have different lengths
        per world, so each set is padded on its own rather than sharing the
        teacher's width.
        """
        ids_list, mask_list, position_ids_list, response_start_idx_list, _ = lists
        padded_ids = torch.nn.utils.rnn.pad_sequence(
            ids_list, batch_first=True, padding_value=self.tokenizer.pad_token_id or 0
        ).to(device)
        ...
        if position_ids_list[0].dim() == 1:
            padded_position_ids = torch.zeros(
                (batch_size, max_len), dtype=position_ids_list[0].dtype, device=device
            )
            for i, position_ids in enumerate(position_ids_list):
                padded_position_ids[i, : position_ids.shape[-1]] = position_ids.to(device)
        else:
            rope_dims = position_ids_list[0].shape[0]
            padded_position_ids = torch.zeros(
                (batch_size, rope_dims, max_len), dtype=position_ids_list[0].dtype, device=device
            )
            for i, position_ids in enumerate(position_ids_list):
                padded_position_ids[i, :, : position_ids.shape[-1]] = position_ids.to(device)
        return (
            ...
            padded_mask,
            padded_position_ids,
            torch.stack(response_start_idx_list).to(device),
        )
    ...
    def _maybe_build_self_distillation_batch(
        ...
        # for both teacher prompt modes. Note: distillation is not reward-free globally;
        # this flag only controls teacher-side input construction.
        use_teacher_always_on_inputs = (
            self_distillation_cfg.get("teacher_always_on", False)
            and self_distillation_cfg.get("teacher_image_key", None) is not None
            and not use_answer_hint
        )
        # Plain distillation: the resolution privilege is given up, and the teacher is
        # shown the student's own image instead of the teacher-resolution column.
        # The teacher still reads `teacher_image_key`, but only for the presence
        # check -- the pixels come from the student's own prompt.
        teacher_use_student_image = self_distillation_cfg.get("teacher_use_student_image", False)
        ...
        if use_answer_hint:
            ...
            # Name of the `extra_info` field holding the generated reasoning chain.
            # Unset (None) keeps the original behaviour of hinting with the
            # ground-truth answer alone.
            answer_hint_key = self_distillation_cfg.get("answer_hint_key", None)
            # Dual-world CWAD: each world carries its OWN image and its OWN hint, and
            # neither side of a world gets a resolution advantage. That needs four
            # distinct prompt sequences -- world A's hint prompt, world B's hint
            # prompt, and world B's plain prompt for the student -- because the two
            # images expand to different numbers of vision tokens and so cannot
            # share one token sequence.
            dual_world = self_distillation_cfg.get("dual_world", False)
            answer_hint_key_b = self_distillation_cfg.get("answer_hint_key_b", None)
            world_b_image_key = self_distillation_cfg.get("teacher_image_key", None)
            if dual_world and not world_b_image_key:
                raise ValueError(
                    "dual-world answer-hint requires self_distillation.teacher_image_key "
                    "to name the world-B image column"
                )
            # The hint and the resolution privilege are independent axes and can be
            # combined: when the privileged columns are present each side of a world
            # takes its own resolution, on top of the hint. Unset keeps the
            # unprivileged behaviour (teacher copies the student's own image, and
            # both sides of world B score the same image).
            privileged_teacher_a_image_key = self_distillation_cfg.get(
                "privileged_teacher_a_image_key", None
            )
            privileged_student_b_image_key = self_distillation_cfg.get(
                "privileged_student_b_image_key", None
            )
            ...
            # Dual-world only: world A's hinted prompt, and world B's plain prompt for
            # the student. The teacher lists above hold world B's hinted prompt.
            teacher_a_lists = [[], [], [], [], []] if dual_world else None
            student_b_lists = [[], [], [], [], []] if dual_world else None
            ...
            for i in range(batch_size):
                ...
                if answer is None and extra_info[i] is not None and isinstance(extra_info[i], dict):
                    answer = extra_info[i].get("answer", None)

                # The hint text is a generated reasoning chain when the dataset
                # carries one (answer_hint_key names the extra_info field), and
                # falls back to the bare ground-truth answer otherwise. It lives in
                # extra_info rather than a new column because extra_info already
                # survives the generation-time key whitelist in _get_gen_batch.
                hint_text = None
                if answer_hint_key and extra_info[i] is not None and isinstance(extra_info[i], dict):
                    hint_text = extra_info[i].get(answer_hint_key, None)
                if hint_text is None or str(hint_text).strip() == "":
                    hint_text = answer

                has_answer = hint_text is not None and str(hint_text).strip() != ""
                ...
                # Privileged combination: the teacher's world-A view is the original
                # image at its own resolution rather than the student's downscaled copy.
                world_a_messages = raw_prompt_messages
                if (
                    dual_world
                    and privileged_teacher_a_image_key
                    and privileged_teacher_a_image_key in batch.non_tensor_batch
                ):
                    world_a_messages = self._prepare_teacher_messages(
                        raw_prompt_messages,
                        self._images_from_batch(batch, privileged_teacher_a_image_key, i),
                    )
                teacher_messages = self._prepare_hint_teacher_messages(
                    world_a_messages,
                    str(hint_text),
                    answer_hint_template,
                )
                ...
                if dual_world:
                    # This set is world A (the student's own image) with world A's hint.
                    ...
                    world_b_images = batch.non_tensor_batch[world_b_image_key][i]
                    if isinstance(world_b_images, np.ndarray):
                        world_b_images = world_b_images.tolist()
                    elif world_b_images is None:
                        world_b_images = []
                    else:
                        world_b_images = list(world_b_images)
                    if not self._teacher_images_available(world_b_images):
                        raise ValueError(
                            f"World-B image column `{world_b_image_key}` is empty for sample {i}."
                        )
                    world_b_messages = self._prepare_teacher_messages(
                        raw_prompt_messages,
                        world_b_images,
                    )

                    hint_b = None
                    if answer_hint_key_b and extra_info[i] is not None and isinstance(extra_info[i], dict):
                        hint_b = extra_info[i].get(answer_hint_key_b, None)
                    if hint_b is None or str(hint_b).strip() == "":
                        hint_b = ""
                    teacher_b_messages = self._prepare_hint_teacher_messages(
                        world_b_messages,
                        str(hint_b),
                        answer_hint_template,
                    )
                    for target, value in zip(
                        (teacher_input_ids_list, teacher_attention_mask_list,
                         teacher_position_ids_list, teacher_response_start_idx_list,
                         teacher_multi_modal_inputs_list),
                        self._build_teacher_prompt_inputs(
                            teacher_b_messages,
                            responses[i],
                    ...
                    # The student scores world B on the plain question -- it gets no hint.
                    # With the privileged column present it sees the half-size edit while
                    # the teacher keeps the native 1024x1024 one; both carry the same
                    # question text and the same response tokens, only the image differs.
                    student_b_messages = world_b_messages
                    if (
                        privileged_student_b_image_key
                        and privileged_student_b_image_key in batch.non_tensor_batch
                    ):
                        student_b_messages = self._prepare_teacher_messages(
                            raw_prompt_messages,
                            self._images_from_batch(batch, privileged_student_b_image_key, i),
                        )
                    for target, value in zip(
                        student_b_lists,
                        self._build_teacher_prompt_inputs(
            ...
            metrics = {
                ...
                "self_distillation/answer_hint_fraction": teacher_present_mask.mean().item(),
            ...
            if dual_world:
                # World A and world B each carry their own prompt prefix, so both extra
                # sets are padded independently of the teacher's.
                for prefix, lists in (("teacher_a", teacher_a_lists), ("student_b", student_b_lists)):
                    ids, mask, position_ids, response_start_idx = self._pad_prompt_list(lists, batch_size, device)
                    tensors[f"{prefix}_input_ids"] = ids
                    tensors[f"{prefix}_attention_mask"] = mask
                    tensors[f"{prefix}_position_ids"] = position_ids
                    tensors[f"{prefix}_response_start_idx"] = response_start_idx
                    non_tensors[f"{prefix}_multi_modal_inputs"] = lists[4]
        ...
        if use_teacher_always_on_inputs:
            ...
            # In dual-world mode the dataset carries a third image column: the counterfactual
            # world-B view that both the student and the teacher score at the same resolution.
            dual_world = self_distillation_cfg.get("dual_world", False)
            world_b_image_key = self_distillation_cfg.get("world_b_image_key", None)
            privileged_teacher_a_image_key = self_distillation_cfg.get(
                "privileged_teacher_a_image_key", None)
            privileged_student_b_image_key = self_distillation_cfg.get(
                "privileged_student_b_image_key", None)
            if dual_world and world_b_image_key:
                if not world_b_image_key:
                    raise ValueError("self_distillation.world_b_image_key is required when dual_world=True.")
                if world_b_image_key not in batch.non_tensor_batch:
                    raise KeyError(
                        f"World-B image key `{world_b_image_key}` not found in batch.non_tensor_batch"
                    )
            ...
            teacher_response_start_idx_list = []
            teacher_multi_modal_inputs_list = []
            world_b_multi_modal_inputs_list = []
            # Privileged variant (A6): the teacher's world-A image and the student's
            # world-B image are separate columns. Each world is tokenized with its OWN
            # image rather than reusing the teacher's token sequence: the images differ
            # in resolution, so they expand to different numbers of vision tokens, and
            # the model rejects a prompt whose image-token count disagrees with the
            # pixel inputs. Only the response tokens are shared across worlds.
            teacher_a_input_ids_list = []
            teacher_a_attention_mask_list = []
            teacher_a_position_ids_list = []
            teacher_a_response_start_idx_list = []
            teacher_a_multi_modal_inputs_list = []
            student_b_input_ids_list = []
            student_b_attention_mask_list = []
            student_b_position_ids_list = []
            student_b_response_start_idx_list = []
            student_b_multi_modal_inputs_list = []
            teacher_present_mask_list = []
            ...
            for i in range(batch_size):
                ...
                if teacher_use_student_image:
                    # Take the image back out of the student's own prompt rather than
                    # from the teacher column: raw_prompt already carries the exact
                    # view the student rolled out on, so the two inputs stay
                    # byte-identical and no resolution privilege is left. The swap
                    # below then re-inserts that same image, which is a no-op.
                    teacher_images = self._extract_images_from_messages(
                ...
                def _prompt_inputs_for(key: str) -> tuple:
                    """Full tokenized inputs for one image column.

                    Returns (input_ids, attention_mask, position_ids,
                    response_start_idx, multi_modal_inputs) built from THIS world's
                    image. It cannot reuse another world's token sequence: a
                    different-resolution image expands to a different number of
                    vision tokens, and the model requires the prompt's image tokens
                    and the pixel inputs to agree. Only the response tokens stay
                    shared -- each world just carries its own prompt prefix.
                    """
                    images = batch.non_tensor_batch[key][i]
                    if isinstance(images, np.ndarray):
                        ...
                        images = list(images)
                    if not self._teacher_images_available(images):
                        raise ValueError(
                            f"Image key `{key}` is empty for sample {i}, but the run requires "
                            "every sample to carry an image for this world."
                        )
                    messages = self._prepare_teacher_messages(
                        list(batch.non_tensor_batch["raw_prompt"][i]),
                        images,
                        teacher_prompt_messages=teacher_prompt_messages,
                    )
                    return self._build_teacher_prompt_inputs(
                        messages,
                        responses[i],
                        response_mask[i],
                        max_prompt_len=self_distillation_cfg.max_reprompt_len,
                ...
                if dual_world and world_b_image_key:
                    world_b_multi_modal_inputs_list.append(
                        _prompt_inputs_for(world_b_image_key)[-1])
                # Only when the dataset actually carries the column: the keys may
                # be set globally while a run uses an older parquet (the plain
                # QC50 build has no privileged columns), and reading a missing
                ...
                if (
                    ...
                    (
                        teacher_a_input_ids,
                        teacher_a_attention_mask,
                        teacher_a_position_ids,
                        teacher_a_response_start_idx,
                        teacher_a_multi_modal_inputs,
                    ) = _prompt_inputs_for(privileged_teacher_a_image_key)
                    teacher_a_input_ids_list.append(teacher_a_input_ids)
                    teacher_a_attention_mask_list.append(teacher_a_attention_mask)
                    teacher_a_position_ids_list.append(teacher_a_position_ids)
                    teacher_a_response_start_idx_list.append(teacher_a_response_start_idx)
                    teacher_a_multi_modal_inputs_list.append(teacher_a_multi_modal_inputs)
                if (
                    dual_world
                    and privileged_student_b_image_key
                    and privileged_student_b_image_key in batch.non_tensor_batch
                ):
                    (
                        student_b_input_ids,
                        student_b_attention_mask,
                        student_b_position_ids,
                        student_b_response_start_idx,
                        student_b_multi_modal_inputs,
                    ) = _prompt_inputs_for(privileged_student_b_image_key)
                    student_b_input_ids_list.append(student_b_input_ids)
                    student_b_attention_mask_list.append(student_b_attention_mask)
                    student_b_position_ids_list.append(student_b_position_ids)
                    student_b_response_start_idx_list.append(student_b_response_start_idx)
                    student_b_multi_modal_inputs_list.append(student_b_multi_modal_inputs)
            ...
            def _pad_prompt_inputs(
                ids_list: list, attention_mask_list: list, position_ids_list: list, response_start_idx_list: list
            ) -> tuple:
                """Pad one world's per-sample prompt inputs into batch tensors."""
                padded_ids = torch.nn.utils.rnn.pad_sequence(
                    ids_list,
            ...
            (# Each privileged world brings its own prompt prefix (see above), so its
             # tensors are padded independently rather than reused from the teacher.
             teacher_input_ids,
             teacher_attention_mask,
             teacher_position_ids,
             teacher_response_start_idx) = _pad_prompt_inputs(
                teacher_input_ids_list,
                teacher_attention_mask_list,
                teacher_position_ids_list,
                teacher_response_start_idx_list,
            ...
            non_tensors = {"teacher_multi_modal_inputs": teacher_multi_modal_inputs_list}
            if dual_world and world_b_image_key:
                non_tensors["world_b_multi_modal_inputs"] = world_b_multi_modal_inputs_list
            if teacher_a_multi_modal_inputs_list:
                non_tensors["teacher_a_multi_modal_inputs"] = teacher_a_multi_modal_inputs_list
            if student_b_multi_modal_inputs_list:
                non_tensors["student_b_multi_modal_inputs"] = student_b_multi_modal_inputs_list
            tensors = {
                "teacher_input_ids": teacher_input_ids,
                "teacher_attention_mask": teacher_attention_mask,
                "teacher_position_ids": teacher_position_ids,
            ...
            if teacher_a_tensors is not None:
                tensors["teacher_a_input_ids"] = teacher_a_tensors[0]
                tensors["teacher_a_attention_mask"] = teacher_a_tensors[1]
                tensors["teacher_a_position_ids"] = teacher_a_tensors[2]
                tensors["teacher_a_response_start_idx"] = teacher_a_tensors[3]
            if student_b_tensors is not None:
                tensors["student_b_input_ids"] = student_b_tensors[0]
                tensors["student_b_attention_mask"] = student_b_tensors[1]
                tensors["student_b_position_ids"] = student_b_tensors[2]
                tensors["student_b_response_start_idx"] = student_b_tensors[3]
            return DataProto.from_dict(tensors=tensors, non_tensors=non_tensors), metrics
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        world_b_image_key = self.config.actor_rollout_ref.actor.get("self_distillation", {}).get("world_b_image_key", None)
        if world_b_image_key and world_b_image_key in batch.non_tensor_batch:
            reward_model_keys.add(world_b_image_key)
        # The strict-privileged dual-world variant carries two more image columns
        # (the teacher's world-A view and the student's world-B view). They are
        # read from the batch after generation, so they must survive this pop --
        # without them the two extra teacher/student forwards silently fall back
        # to `teacher_image_key` and both worlds score the same image.
        self_distillation_cfg = self.config.actor_rollout_ref.actor.get("self_distillation", {})
        for key in (
            self_distillation_cfg.get("privileged_teacher_a_image_key", None),
            self_distillation_cfg.get("privileged_student_b_image_key", None),
        ):
            if key and key in batch.non_tensor_batch:
                reward_model_keys.add(key)
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    reward_free_teacher_cwad = self._use_reward_free_teacher_cwad()
                    reward_extra_infos_dict = {}
                    if reward_free_teacher_cwad:
                        reward_tensor = torch.zeros_like(batch.batch["responses"], dtype=torch.float32)
                    else:
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if not reward_free_teacher_cwad and self.config.reward_model.launch_reward_fn_async:
                        ...
                        # A reward-free teacher step never consumes real rewards or KL-in-reward.
                        if reward_free_teacher_cwad:
                            pass
                        ...
                        self_distillation_cfg = self.config.actor_rollout_ref.actor.get("self_distillation", None)
                        loss_mode = self.config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla")
                        skip_advantage_for_cwad = self_distillation_cfg is not None and loss_mode == "cwad"
                        if skip_advantage_for_cwad and "self_distillation_mask" in batch.batch.keys():
                            skip_advantage_for_cwad = bool(torch.all(batch.batch["self_distillation_mask"] > 0.5).item())

                        if not skip_advantage_for_cwad:
