...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        self_distillation_needs_ref = self_distillation_cfg is not None and loss_mode == "cwad"
    ...
    def run(self, config):
        ...
        custom_chat_template = resolve_custom_chat_template(config.actor_rollout_ref.model)
        if custom_chat_template is not None:
            if processor is not None:
                processor.chat_template = custom_chat_template
            tokenizer.chat_template = custom_chat_template

        self_distillation_cfg = config.actor_rollout_ref.actor.get("self_distillation", {})
        reward_free_teacher_cwad = (
            config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla") == "cwad"
