...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        Distillation is enabled when policy_loss.loss_mode is "cwad".
        ...
        gamma (float): Weight applied to the distillation loss.
        ...
        distillation_objective (str): Distributional objective. ``generalized_jsd`` preserves the
            original alpha-interpolated KL/JSD behavior; ``cwad_topk_reverse_kl`` implements the
            bias-corrected top-k reverse KL used by CWAD.
        distillation_topk_source (str): Policy that chooses the top-k support. Options: ``student``
            (the historical behaviour) or ``teacher`` (required by the CWAD top-k objective).
        ...
        teacher_use_student_image (bool): When True, ignore the pixels in teacher_image_key and give the teacher the
            student's own image, taken from the student's prompt. This removes the resolution privilege, leaving one
            model against another on identical inputs. The column is still read for the presence check.
        world_b_image_key (Optional[str]): Dataset column holding the world-B (counterfactual) images. In dual-world
            mode this single image is scored by both the teacher and the student, while teacher_image_key holds the
            original-resolution world-A view of the teacher.
        ...
        dual_world (bool): Whether to use the cross-world CWAD objective: the dataset carries two
            visual worlds (world A = the original image, world B = a counterfactual edit) and all
            four student/teacher x world A/B distributions are gathered on the union of the two
            teachers' top-k supports. World A is seen at student resolution by the student
            (data.image_key) and at original resolution by the teacher (teacher_image_key); both
            sides see the same world-B image (world_b_image_key).
        privileged_teacher_a_image_key (Optional[str]): Dataset column holding the
            teacher's world-A image at its own resolution (the strict privileged CWAD
            variant: the student sees world A at student resolution, the teacher at
            original resolution). None keeps the unprivileged behaviour where both
            sides score the same image.
        privileged_student_b_image_key (Optional[str]): Dataset column holding the
            student's world-B image (a half-size copy of the counterfactual edit), so
            world B carries the same resolution privilege as world A.
        cwad_lambda (float): Weight of the belief-transition term of the CWAD loss.
        cwad_tau (float): Temperature of the softmax used to compare the two worlds' belief shifts.
    """
    ...
    distillation_objective: str = "generalized_jsd"
    ...
    teacher_use_student_image: bool = False
    world_b_image_key: Optional[str] = None
    teacher_prompt_mode: Optional[str] = None
    answer_hint_key: Optional[str] = None
    answer_hint_key_b: Optional[str] = None
    ...
    dual_world: bool = False
    privileged_teacher_a_image_key: Optional[str] = None
    privileged_student_b_image_key: Optional[str] = None
    cwad_lambda: float = 1.0
    cwad_tau: float = 1.0
    ...
    def __post_init__(self):
        ...
        if self.distillation_objective not in valid_distillation_objectives:
            raise ValueError(
                "self_distillation.distillation_objective must be one of "
                f"{valid_distillation_objectives}, got {self.distillation_objective}"
            )
        valid_topk_sources = ["student", "teacher"]
        if self.distillation_topk_source not in valid_topk_sources:
            raise ValueError(
                "self_distillation.distillation_topk_source must be one of "
                f"{valid_topk_sources}, got {self.distillation_topk_source}"
            )
        if self.distillation_objective == "cwad_topk_reverse_kl":
            if not self.full_logit_distillation or self.distillation_topk is None:
                raise ValueError(
                    "cwad_topk_reverse_kl requires full_logit_distillation=True and distillation_topk to be set."
                )
            if self.distillation_topk_source != "teacher":
                raise ValueError("cwad_topk_reverse_kl requires distillation_topk_source='teacher'.")
            if self.distillation_add_tail:
                raise ValueError("cwad_topk_reverse_kl requires distillation_add_tail=False.")
            if self.alpha != 1.0:
                raise ValueError("cwad_topk_reverse_kl requires alpha=1.0.")
        ...
        if self.cwad_tau <= 0:
            raise ValueError(f"self_distillation.cwad_tau must be positive, got {self.cwad_tau}")
        if self.dual_world:
            if not self.full_logit_distillation:
                raise ValueError("self_distillation.dual_world requires full_logit_distillation=True.")
            if self.distillation_topk is None:
                raise ValueError("self_distillation.dual_world requires distillation_topk to be set.")
            if self.distillation_topk_source != "teacher":
                raise ValueError("self_distillation.dual_world requires distillation_topk_source='teacher'.")
            # world_b_image_key is optional. With it, the dataset carries a third
            # image column and each side reads its own world-B image. Without it
            # the two-image layout is used (images = world A, teacher_image_key =
            # world B), which is what the current dual-world runs train on.
...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg', 'cwad'.
