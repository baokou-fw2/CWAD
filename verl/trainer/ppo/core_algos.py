...
def _bias_corrected_kl(student_logp: torch.Tensor, teacher_logp: torch.Tensor, mask: Optional[torch.Tensor] = None):
    """sum_v p_s log(p_s/p_t) - p_s + p_t over the last dim.

    The Bregman divergence used on the teacher's top-k support:
    non-negative, zero iff the two distributions agree, and well behaved when the
    support misses teacher mass. ``mask`` selects which support slots count.
    """
    student_logp = student_logp.float()
    teacher_logp = teacher_logp.float()
    student_probs = student_logp.exp()
    teacher_probs = teacher_logp.exp()
    per_slot = student_probs * (student_logp - teacher_logp) + (teacher_probs - student_probs)
    if mask is not None:
        per_slot = per_slot * mask
    return per_slot.sum(-1)


def _centered_log_probs(logp: torch.Tensor) -> torch.Tensor:
    """phi(p) = log p - mean(log p) over the support (Eq. 4)."""
    centered = logp.float()
    return centered - centered.mean(dim=-1, keepdim=True)


def compute_cwad_loss(
    student_a_topk_log_probs: torch.Tensor,
    student_b_topk_log_probs: torch.Tensor,
    teacher_a_topk_log_probs: torch.Tensor,
    teacher_b_topk_log_probs: torch.Tensor,
    member_a: torch.Tensor,
    member_b: torch.Tensor,
    response_mask: torch.Tensor,
    cwad_lambda: float,
    cwad_tau: float,
    ...
    """Cross-World Aligned Distillation.

        L_world = KL(p^A_S || p^A_T) + KL(p^B_S || p^B_T)   (each on its own teacher top-k)
        Delta   = phi(p^A) - phi(p^B);  r = softmax(Delta / tau)
        L_trans = KL(r_S || r_T)                             (on the union support)
        L_CWAD  = L_world + lambda * L_trans

    All four distributions are gathered on the union of the two worlds' teacher
    top-k supports. ``member_a``/``member_b`` mark which union slots came from
    world A's and world B's own support, so each L_world term is restricted to
    that world's teacher top-k even though it is computed on the union.
    """
    if cwad_tau <= 0:
        raise ValueError(f"cwad_tau must be positive, got {cwad_tau}")

    loss_mask = response_mask
    if self_distillation_mask is not None:
        loss_mask = loss_mask * self_distillation_mask.unsqueeze(1)

    mask_a = member_a.to(torch.float32)
    mask_b = member_b.to(torch.float32)

    # --- L_world: reverse KL applied independently in each world ------------
    world_a_per_token = _bias_corrected_kl(student_a_topk_log_probs, teacher_a_topk_log_probs, mask_a)
    world_b_per_token = _bias_corrected_kl(student_b_topk_log_probs, teacher_b_topk_log_probs, mask_b)
    world_per_token = world_a_per_token + world_b_per_token

    # --- L_trans: belief transition over the union support ------------------
    # Rows in one batch can have differently sized unions: padding slots repeat a
    # real member, so they must be excluded from the softmax or they double-count.
    slot_valid = member_a | member_b
    neg_inf = torch.finfo(torch.float32).min
    delta_s = _centered_log_probs(student_a_topk_log_probs) - _centered_log_probs(student_b_topk_log_probs)
    delta_t = _centered_log_probs(teacher_a_topk_log_probs) - _centered_log_probs(teacher_b_topk_log_probs)
    delta_s = delta_s.masked_fill(~slot_valid, neg_inf)
    delta_t = delta_t.masked_fill(~slot_valid, neg_inf)
    response_s = torch.softmax(delta_s / cwad_tau, dim=-1)
    response_t = torch.softmax(delta_t / cwad_tau, dim=-1)
    trans_per_token = _bias_corrected_kl(response_s.clamp_min(1e-30).log(), response_t.clamp_min(1e-30).log())

    raw_per_token_loss = world_per_token + cwad_lambda * trans_per_token

    weighted_per_token_loss = raw_per_token_loss
    if is_clip is not None:
        # The union is sorted ascending, so no slot is "the sampled token" the way
        # slot 0 is in the single-world descending top-k. Use the student's own
        # log-prob of the sampled token for the importance ratio.
        if student_log_probs is None or old_log_probs is None:
            raise ValueError("student_log_probs and old_log_probs are required for the distillation IS ratio.")
    ...
    def _masked_mean(tensor: torch.Tensor) -> float:
        return (verl_F.masked_sum(tensor, loss_mask) / valid_token_count).detach().item()

    metrics["self_distillation/cwad_world_token_mean"] = _masked_mean(world_per_token)
    metrics["self_distillation/cwad_world_a_token_mean"] = _masked_mean(world_a_per_token)
    metrics["self_distillation/cwad_world_b_token_mean"] = _masked_mean(world_b_per_token)
    metrics["self_distillation/cwad_trans_token_mean"] = _masked_mean(trans_per_token)
    metrics["self_distillation/cwad_total_token_mean"] = _masked_mean(raw_per_token_loss)
    metrics["self_distillation/raw_distillation_token_mean"] = metrics["self_distillation/cwad_total_token_mean"]
    metrics["self_distillation/weighted_jsd_token_mean"] = _masked_mean(weighted_per_token_loss)
    metrics["self_distillation/cwad_lambda"] = cwad_lambda
    metrics["self_distillation/cwad_tau"] = cwad_tau
    metrics["self_distillation/cwad_support_size"] = float(member_a.size(-1))
    metrics["self_distillation/self_distillation_mask.mean()"] = (
        1.0 if self_distillation_mask is None else self_distillation_mask.float().mean().detach().item()
    )
...
def compute_self_distillation_loss(
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        distillation_objective = getattr(
            self_distillation_config, "distillation_objective", "generalized_jsd"
        )
        if use_topk:
            ...
            if distillation_objective == "cwad_topk_reverse_kl":
                if self_distillation_config.distillation_add_tail:
                    raise ValueError("cwad_topk_reverse_kl does not support a tail bucket.")

                # Bias-corrected reverse KL: sum_v p_s(v) log[p_s(v)/p_t(v)] - p_s(v) + p_t(v).
                # Keep the original full-softmax probabilities on the teacher top-k support: no
                # tail bucket and no top-k renormalization.
                student_logp_fp32 = student_distill_log_probs.float()
                teacher_logp_fp32 = teacher_distill_log_probs.float()
                student_probs = student_logp_fp32.exp()
                teacher_probs = teacher_logp_fp32.exp()
                reverse_kl_term = student_probs * (student_logp_fp32 - teacher_logp_fp32)
                bias_correction = teacher_probs - student_probs
                raw_per_token_loss = (reverse_kl_term + bias_correction).sum(-1)
                diagnostic_tensors = {
                    "self_distillation/teacher_topk_mass_mean": teacher_probs.sum(-1),
                    "self_distillation/student_on_teacher_topk_mass_mean": student_probs.sum(-1),
                    "self_distillation/cwad_reverse_kl_term_mean": reverse_kl_term.sum(-1),
                    "self_distillation/cwad_bias_correction_mean": bias_correction.sum(-1),
                }
        ...
        if not (use_topk and distillation_objective == "cwad_topk_reverse_kl"):
            if self_distillation_config.alpha == 0.0:
                ...
                kl_teacher = F.kl_div(
                    mixture_log_probs, teacher_distill_log_probs, reduction="none", log_target=True
                )
                kl_student = F.kl_div(
                    mixture_log_probs, student_distill_log_probs, reduction="none", log_target=True
                )
                kl_loss = torch.lerp(
                    kl_student, kl_teacher, alpha
    ...
    metrics["self_distillation/raw_distillation_token_mean"] = metrics[
    ...
    metrics["self_distillation/num_distill_tokens"] = loss_mask.sum().detach().item()
    for metric_name, metric_tensor in diagnostic_tensors.items():
        metrics[metric_name] = (
            verl_F.masked_sum(metric_tensor, loss_mask) / valid_token_count
        ).detach().item()
